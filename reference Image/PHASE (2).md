...
## Phase 8 — Landing Page Visual Redesign (Presentation Layer Only)

**Direction note:** the reference changed after an initial pass at a 3D/Web3 hero
concept — the current, authoritative direction is a minimal, premium, dark hero with no
header, described in full in `REDESIGN_PLAYBOOK.md`. If anything here conflicts with that
file, `REDESIGN_PLAYBOOK.md` wins.

**Goal:** Rebuild the landing page as a clean, premium, minimal hero (no header/nav)
matching a supplied reference image, with a swappable background layer that will later
take a looping video — without touching anything that already works.

Checklist:
- [ ] Reference image reviewed before any code changes.
- [ ] Previous landing-page visual/marketing code removed; existing functional
      components (wallet connect, circuit-call form, public state panel) preserved
      untouched and simply re-mounted into the new design.
- [ ] No header/top navigation — hero content only.
- [ ] Background built as its own swappable layer (not baked into the text markup), with
      a dark scrim/overlay between it and the text, ready to take a video later without
      further restructuring.
- [ ] Wallet connect/disconnect, circuit call, and public state panel behavior verified
      unchanged, using the regression flow in `LAUNCH_PLAYBOOK.md` Step 5.
- [ ] No contract, test, CI, or env-wiring file touched.
- [ ] Copy is our project's real content (from `PROJECT.md`), not the reference's
      original text.
- [ ] Commits made per section (`RULES.md` §2), `MEMORY.md` updated.

**Definition of done:** hero matches the reference's layout, typography, and color
language with our own copy; the full functional regression check passes; nothing outside
the presentation layer changed. Full prompts (initial build + later video swap) in
`REDESIGN_PLAYBOOK.md`.

> **Direction update:** the visual reference changed from the 3D/neon Web3 style in
> `REDESIGN_PLAYBOOK.md` to a minimal, high-whitespace "premium" hero style. The current
> authoritative prompt for Phase 8 is `PREMIUM_HERO_PLAYBOOK.md` — treat `REDESIGN_PLAYBOOK.md`
> as superseded unless you deliberately want to combine elements of both.
