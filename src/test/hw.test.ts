...
const PRIVATE_STATE_ID = 'AlicePrivateAllowlistState';
...
describe(`Private Allowlist Contract (${network})`, () => {
  ...
  it('Deploys the contract', async () => {
    ...
    const deployed: DeployedContract<Contract> =
      await (deployContract<Contract>)(providers, {
        compiledContract: CompiledPrivateAllowlistContract,
    ...
    expect(state.allowlistCommitment).toEqual(0n);
    expect(state.verifiedCount).toEqual(0n);
    expect(state.lastActionStatus).toEqual(false);
  });
  ...
  it('Publishes allowlist commitment', async () => {
    ...
    await (submitCallTx<Contract, 'publishCommitment'>)(providers, {
      ...
      contractAddress,
      privateStateId: PRIVATE_STATE_ID,
      circuitId: 'publishCommitment',
      args: [commitment],
    });

    const state = await queryLedger(providers);
    expect(state.allowlistCommitment).toEqual(commitment);
    expect(state.verifiedCount).toEqual(0n);
  ...
  it('Verifies membership using private witness', async () => {
    ...
    const state = await queryLedger(providers);
    expect(state.allowlistCommitment).toEqual(commitment);
    expect(state.verifiedCount).toEqual(1n);
    expect(state.lastActionStatus).toEqual(true);
  });
});
